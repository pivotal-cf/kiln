	...
	Describe("Execute", func() {
		Context("when no command name is given", func() {
			It("prints the global usage to the output", func() {
				cook := &fakes.Command{}
				cook.UsageReturns(jhandacommands.Usage{ShortDescription: "cooks you a stew"})

				clean := &fakes.Command{}
				clean.UsageReturns(jhandacommands.Usage{ShortDescription: "cleans the pot you used"})
		...
		Context("when a command name is given", func() {
			It("prints the usage for that command", func() {
				cook := &fakes.Command{}
				cook.UsageReturns(jhandacommands.Usage{
				...
				help := commands.NewHelp(output, strings.TrimSpace(flags), jhandacommands.Set{"cook": cook})
			...
			Context("when the command flags cannot be determined", func() {
				It("returns an error", func() {
					...
					help := commands.NewHelp(output, flags, jhandacommands.Set{"cook": cook})
					err := help.Execute([]string{"cook"})
