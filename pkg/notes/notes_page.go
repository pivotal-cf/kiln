...
func ParseNotesPage(input string) (Page, error) {
	return parseNotesPageWithExpressionAndReleasesSentinel(input, releaseNoteExp.String(), DefaultReleasesSentinel)
}

func parseNotesPageWithExpressionAndReleasesSentinel(input, releaseRegularExpression, releasesSentinel string) (Page, error) {
	const (
	...
	default:
		...
		for _, match := range matchStrings {
			page.Releases = append(page.Releases, TileVersionNote{
...
func (page *Page) validateRelease(tile TileVersionNote) error {
	_, err := tile.version()
	if err != nil {
...
func (page *Page) Add(versionNote TileVersionNote) error {
	...
	if len(page.Releases) == 0 {
		page.Releases = []TileVersionNote{versionNote}
		return nil
	}
	...
	for i, t := range page.Releases {
		...
		page.Releases = append(page.Releases[:i], append([]TileVersionNote{versionNote}, page.Releases[i:]...)...)
		return nil
	}
...
type TileVersionNote struct {
	Version string
	Notes   string
}

func (notes TileVersionNote) version() (*semver.Version, error) {
