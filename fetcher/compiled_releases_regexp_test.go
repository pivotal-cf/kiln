...
var _ = Describe("ReleasesRegexp", func() {
	var (
		...
		regex           *ReleasesRegexp
		err             error
	)

	It("takes a regex string and converts it to a CompiledRelease", func() {
		regex, err = NewReleasesRegexp(`^2.5/.+/(?P<release_name>[a-z-_]+)-(?P<release_version>[0-9\.]+)-(?P<stemcell_os>[a-z-_]+)-(?P<stemcell_version>[\d\.]+)\.tgz$`)
		Expect(err).NotTo(HaveOccurred())

		compiledRelease, err = regex.Convert("2.5/uaa/uaa-1.2.3-ubuntu-trusty-123.tgz")
		Expect(err).NotTo(HaveOccurred())
		Expect(compiledRelease).To(Equal(CompiledRelease{ID: ReleaseID{Name: "uaa", Version: "1.2.3"}, StemcellOS: "ubuntu-trusty", StemcellVersion: "123"}))
	})
	...
	It("returns an error if a required capture is missing", func() {
		regex, err = NewReleasesRegexp(`^2.5/.+/([a-z-_]+)-(?P<release_version>[0-9\.]+)-(?P<stemcell_os>[a-z-_]+)-(?P<stemcell_version>[\d\.]+)\.tgz$`)
		Expect(err).To(HaveOccurred())
