...
var _ = Describe("MetadataPartsDirectoryReader", func() {
	...
	BeforeEach(func() {
		var err error
		tempDir, err = os.MkdirTemp("", "metadata-parts-directory-reader")
	...
	Describe("Read", func() {
		BeforeEach(func() {
			err := os.WriteFile(filepath.Join(tempDir, "vars-file-1.yml"), []byte(`---
- name: variable-1
  type: certificate
- name: variable-2
...
`), 0o755)
			Expect(err).ToNot(HaveOccurred())
			err = os.WriteFile(filepath.Join(tempDir, "vars-file-2.yml"), []byte(`---
name: variable-3
type: password
`), 0o755)
			Expect(err).ToNot(HaveOccurred())
			err = os.WriteFile(filepath.Join(tempDir, "ignores.any-other-extension"), []byte("not-yaml"), 0o755)
			Expect(err).ToNot(HaveOccurred())
		...
		Context("when there is an error reading from a file", func() {
			It("errors", func() {
				err := os.WriteFile(filepath.Join(tempDir, "unreadable-file.yml"), []byte(`unused`), 0o000)
				Expect(err).ToNot(HaveOccurred())
		...
		Context("when a yaml file is malformed", func() {
			It("errors", func() {
				err := os.WriteFile(filepath.Join(tempDir, "not-valid-yaml.yml"), []byte(`{{`), 0o755)
				Expect(err).ToNot(HaveOccurred())
		...
		Context("when file contains an array item without a name", func() {
			BeforeEach(func() {
				err := os.WriteFile(filepath.Join(tempDir, "vars-file-1.yml"), []byte(`[{foo: bar}]`), 0o755)
				Expect(err).ToNot(HaveOccurred())
			})
	...
	Context("when a top-level key is specified", func() {
		BeforeEach(func() {
			err := os.WriteFile(filepath.Join(tempDir, "_order.yml"), []byte(`---
variable_order:
- variable-3
- variable-2
- variable-1
...
`), 0o755)
		...
		Context("failure cases", func() {
			Context("when a yaml file does not contain the top-level key", func() {
				It("errors", func() {
					err := os.WriteFile(filepath.Join(tempDir, "not-a-vars-file.yml"), []byte(`constants: []`), 0o755)
					Expect(err).ToNot(HaveOccurred())
		...
		Context("when variable file is neither a slice or a map", func() {
			BeforeEach(func() {
				err := os.WriteFile(filepath.Join(tempDir, "vars-file-1.yml"), []byte(`variables: foo`), 0o755)
				Expect(err).ToNot(HaveOccurred())
			})
		...
		Context("when variable file contains an array item without a name", func() {
			BeforeEach(func() {
				err := os.WriteFile(filepath.Join(tempDir, "vars-file-1.yml"), []byte(`variables: [{foo: bar}]`), 0o755)
				Expect(err).ToNot(HaveOccurred())
			})
		...
		Context("when specifying an Order key", func() {
			...
			Context("failure cases", func() {
				...
				Context("when _order.yml file is not in valid format", func() {
					BeforeEach(func() {
						err := os.WriteFile(filepath.Join(tempDir, "_order.yml"), []byte(`variable_order: foo`), 0o755)
						Expect(err).ToNot(HaveOccurred())
					})
				...
				Context("when _order.yml file contains a name that does not exist", func() {
					BeforeEach(func() {
						err := os.WriteFile(filepath.Join(tempDir, "_order.yml"), []byte(`variable_order: [some-file]`), 0o755)
						Expect(err).ToNot(HaveOccurred())
					})
