	...
	Context("Execute", func() {
		...
		When("updating to a version that exists in the remote", func() {
			It("downloads the release", func() {
				...
				_, _, receivedReleaseRequirement := releaseSource.GetMatchedReleaseArgsForCall(0)
				...
				_, _, receivedReleasesDir, receivedRemoteRelease := releaseSource.DownloadReleaseArgsForCall(0)
		...
		When("updating lock file without downloading", func() {
			It("writes the new version to the Kilnfile.lock", func() {
				...
				_, _, receivedReleaseRequirement := releaseSource.FindReleaseVersionArgsForCall(0)
				releaseRequirement := component.Spec{
					Name:             releaseName,
					Version:          newReleaseVersion,
					StemcellOS:       "some-os",
