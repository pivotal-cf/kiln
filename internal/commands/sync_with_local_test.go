	...
	Describe("Execute", func() {
		...
		BeforeEach(func() {
			...
			localReleaseDirectory.GetLocalReleasesReturns([]component.Local{
				{
					Lock:      cargo.BOSHReleaseTarballLock{Name: release1Name, Version: release1NewVersion, SHA1: release1NewSha},
			...
			remotePather.RemotePathCalls(func(requirement cargo.BOSHReleaseTarballSpecification) (path string, err error) {
		...
		It("updates the Kilnfile.lock to have the same version as the local releases", func() {
			...
			Expect(updatedLockfile.Releases).To(Equal([]cargo.BOSHReleaseTarballLock{
				{
					Name:         release1Name,
		...
		When("one of the releases on disk is the same version as in the Kilnfile.lock", func() {
			BeforeEach(func() {
				localReleaseDirectory.GetLocalReleasesReturns([]component.Local{
					{
						Lock:      cargo.BOSHReleaseTarballLock{Name: release1Name, Version: release1OldVersion, SHA1: release1NewSha},
						LocalPath: "local-path",
					},
					{
						Lock:      cargo.BOSHReleaseTarballLock{Name: releaseName, Version: releaseNewVersion, SHA1: releaseNewSha},
