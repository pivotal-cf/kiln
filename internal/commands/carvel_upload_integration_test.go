...
// Integration specs for CarvelUpload; drive the real bosh CLI end to end.
var _ = Describe("CarvelUpload (integration)", func() {
	var (
	...
	Describe("Execute", func() {
		When("valid Kilnfile is provided with a round-trip mock Artifactory", func() {
			...
			It("uploads the BOSH release and writes a standard Kilnfile.lock", func() {
				...
				Expect(lock.Releases[0].Name).To(Equal("k8s-tile-test-pkg"))
				...
				Expect(lock.Releases[0].RemotePath).To(ContainSubstring("k8s-tile-test-pkg"))
