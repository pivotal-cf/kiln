...
var _ = Describe("UpdateStemcell", func() {
	...
	Describe("Execute", func() {
		...
		BeforeEach(func() {
			...
			releaseSource.GetMatchedReleaseCalls(func(_ context.Context, requirement component.Spec) (component.Lock, error) {
			...
			releaseSource.DownloadReleaseCalls(func(_ context.Context, _ string, remote component.Lock) (component.Local, error) {
				switch remote.Name {
				case release1Name:
		...
		It("looks up the correct releases", func() {
			...
			_, req1 := releaseSource.GetMatchedReleaseArgsForCall(0)
			...
			_, req2 := releaseSource.GetMatchedReleaseArgsForCall(1)
		...
		It("downloads the correct releases", func() {
			...
			_, actualDir, remote1 := releaseSource.DownloadReleaseArgsForCall(0)
			...
			_, actualDir, remote2 := releaseSource.DownloadReleaseArgsForCall(1)
			Expect(actualDir).To(Equal(releasesDirPath))
		...
		When("the remote information for a release doesn't change", func() {
			...
			It("doesn't download the release", func() {
				...
				_, _, remote := releaseSource.DownloadReleaseArgsForCall(0)
