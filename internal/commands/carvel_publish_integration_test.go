...
// Integration specs for CarvelPublish; drive the real bosh CLI end to end.
var _ = Describe("CarvelPublish (integration)", func() {
	var (
		...
		When("--final flag is used with a round-trip mock Artifactory", func() {
			...
			BeforeEach(func() {
				...
				for _, cmd := range []*exec.Cmd{
					exec.Command("git", "add", "."),
					exec.Command("git", "-c", "user.name=test", "-c", "user.email=test@test.com", "commit", "-m", "add kilnfiles"),
				} {
					cmd.Dir = inputPath
					out, err := cmd.CombinedOutput()
					Expect(err).NotTo(HaveOccurred(), "error invoking git: "+string(out))
