...
releases based on information from both the assets file and an assets lock file.
The assets lock file name is expected to be a file in the same directory as the
specified assets file with `lock` instead of `yaml` as the filename extension.
...
checksum specified for the release in the assets lock file. If the checksums do
...
releases with different hashes this may result in some problems for local
development.*
...
- `regex:`: is used to specify a capture groups to map release and stemcell
  data to an object in the S3 bucket. The following capture groups must be in
  the regex. The regex capture group maps to fields in the assets lock file.
  **All of the following capture groups must be in the RegEx specified in the
  assets file**
  - `release_name` must map to the assets.lock file under `releases[].name`
  - `release_version` must map to the assets.lock file under
    `releases[].version`
  - `stemcell_os` must map to the assets.lock file under `stemcell_criteria.os`
  - `stemcell_version` must map to the assets.lock file under
    `stemcell_criteria.version`

### Assets Lock

Currently the releases for assets lock file is can not be generated by kiln.
...
Engineering we use a task in our CI to generate this file.

The file has two top level members `releases` and `stemcell_criteria`.

The `releases` member is an array of members with each element having the
having the following members.
- `name`: bosh release name
- `sha1`: checksum of the tarball
- `version`: semantic version of the release

The `stemcell_criteria ` member is an array of members with each element having the
having the following members.
- `name`: bosh release name
- `sha1`: checksum of the tarball
- `version`: semantic version of the release
...
*Storing credentials like S3 keys in a repo is probably not the best idea, so
kiln fetch supports interpolating those variables from a file (`--variables-file`)
or as flags (`--variable`).*
