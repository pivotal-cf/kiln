...
- `regex:`: a regular expression applied to the full-path directory listing of
  ...
  - `release_version` must map to the assets.lock file under
    `releases[].version`
  - `stemcell_os` may map to the assets.lock file under `stemcell_criteria.os`
  - `stemcell_version` may map to the assets.lock file under
...
Currently the releases for the assets lock file is can not be generated by kiln.
...
Engineering we use a consourse task in our CI to generate this the assets lock
file.
